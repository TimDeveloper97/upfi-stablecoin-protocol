...
async function approve() {
  ...
  let result = await upfi.connect(deployer).approve(POOL_CONTRACT_ADDRESS, ethers.utils.parseUnits("1000000", 18));
  console.log("Upfi result: ", result)


  const Usdc = await ethers.getContractFactory("Usdc");
  const usdc = Usdc.attach(USDC);
  let result1 = await usdc.connect(deployer).approve(POOL_CONTRACT_ADDRESS, ethers.utils.parseUnits("1000000", 18));
  console.log("Usdc result: ", result1)

  const Upo = await ethers.getContractFactory("Upfi");
  const upo = Upo.attach(UPO);
  let result2 = await upo.connect(deployer).approve(POOL_CONTRACT_ADDRESS, ethers.utils.parseUnits("1000000", 18));
