...
const ETH_CONTRACT_ADDRESS = "0x627670a7f376E6f8B502Bce7D2bbbFEdAa7cCaa8"
...
async function deploy() {
  ...
  const eth = await Eth.deploy("BTC testnet (UPFI Network)", "BTC");
  await eth.deployed();
  console.log("Eth deployed to:", eth.address);
}

async function initialize() {
  const MyContract = await ethers.getContractFactory("Eth");
